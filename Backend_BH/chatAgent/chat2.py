...
from tools.search_tool2 import search_tool as recommender_tool
...
def addNewData(client_ref: str, new_data: dict):
    """Ajoute des nouvelles données au document client existant"""
    try:
        client = get_mongodb_client()
        db = client[MONGODB_DBNAME]
        collection = db["client_recommendations"]
        ...
        if not last_doc:
            raise HTTPException(status_code=404, detail=f"Aucun document trouvé pour client_ref={client_ref}")
        
        # Mise à jour du dernier document trouvé
        result = collection.update_one(
            {"_id": last_doc["_id"]},
            {"$set": {"new_data": new_data, "updated_at": datetime.now()}}
        )

        if result.modified_count > 0:
            return {"status": "success", "message": f"Nouvelles données ajoutées pour client_ref={client_ref}"}
        else:
            return {"status": "warning", "message": "La mise à jour n'a pas eu lieu (données déjà identiques ?)"}
...
async def chat_with_agent(request: ChatRequest):
    try:
        ...
        conversation_history = [
            {
                ...
                "content": (
                    ...
                    "1. Si des informations sont manquantes ou incertaines, demande-les au client avant de répondre si le client vous fournie appelle tool 'addNewData' pour ajouter au base de données.\n"
                    ...
                    "3. si le client est intéressé par le produit recommandé, appelle `update_status` pour changer le statut en 'accepted' .\n"
        ...
        try:
            llm = get_llm_chat()
            tools = [recommender_tool, update_status, save_recommendation, addNewData]
