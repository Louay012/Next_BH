...
function RefusedRecommendations() {
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    fetch("http://127.0.0.1:5000/list_refused")
      .then((res) => res.json())
      .then((resData) => {
        setData(resData);
        setLoading(false);
    ...
    <div className="p-6 bg-gray-100 min-h-screen mt-20">
      ...
      <div className="bg-white shadow-md rounded-lg p-4 mb-6">
        <p className="text-lg font-semibold text-gray-700">
          ❌ Total Refusées :{" "}
          <span className="text-red-600">{data.total_refused}</span>
        </p>
      </div>
      ...
      <div className="bg-white shadow-md rounded-lg p-4">
        ...
        <table className="w-full border-collapse border border-gray-300">
          ...
          <tbody>
            {data.refused_list.map((rec, index) => (
...
export default RefusedRecommendations;
